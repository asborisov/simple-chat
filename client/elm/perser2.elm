...
import Html exposing (br, div, text, Html)
...
decoder type_ =
  case type_ of
    ...
    "login" -> loginDecoder
    _ -> Json.Decode.fail "Unknown type"

parseMessage : String -> ReturnResult
parseMessage input =
  let
    messageType = Result.withDefault "" (Json.Decode.decodeString (Json.Decode.field "type" Json.Decode.string) input)
  in
      input
...
resultToString result =
  case result of
    MessageRes msg -> "ts: " ++ (toString msg.timeStamp) ++ "text: " ++ msg.text
    LoginRes login -> "User " ++ login.user ++ " logged in: " ++ login.text
    Error err -> err
