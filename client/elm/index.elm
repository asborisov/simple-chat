...
parseResponse response =
  let
    msgType = log "type" (Json.Decode.decodeString (Json.Decode.field "type" Json.Decode.string) response)
    msgStatus = log "status" (Json.Decode.decodeString (Json.Decode.field "status" Json.Decode.string) response)
  in
    case msgType of
      "login" ->
        case msgStatus of
...
update msg model =
  case msg of
    SEND_LOGIN ->
      let
        payload = { username = "timan", guid = "123" }
      in
    ...
    SOCKET_MESSAGE response ->
      let
        msgType = log "type" (Json.Decode.decodeString (Json.Decode.field "type" Json.Decode.string) response)
        msgData = log "data" (Json.Decode.decodeString (Json.Decode.field "payload" Json.Decode.value) response)
      in
        (model, Cmd.none)
